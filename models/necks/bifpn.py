...
class BiFPNBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        p7_td = p7_x
        p6_td = self.p6_td(w1[0, 0] * p6_x + w1[1, 0] * F.interpolate(p7_td, size=p6_x.shape[2:], mode="bilinear", align_corners=True))      
        p5_td = self.p5_td(w1[0, 1] * p5_x + w1[1, 1] * F.interpolate(p6_td, size=p5_x.shape[2:], mode="bilinear", align_corners=True))
        p4_td = self.p4_td(w1[0, 2] * p4_x + w1[1, 2] * F.interpolate(p5_td, size=p4_x.shape[2:], mode="bilinear", align_corners=True))
        p3_td = self.p3_td(w1[0, 3] * p3_x + w1[1, 3] * F.interpolate(p4_td, size=p3_x.shape[2:], mode="bilinear", align_corners=True))

        # Calculate Bottom-Up Pathway
        p3_out = p3_td
        p4_out = self.p4_out(w2[0, 0] * p4_x + w2[1, 0] * p4_td + w2[2, 0] * F.interpolate(p3_out, size=p4_x.shape[2:], mode="bilinear", align_corners=True))
        p5_out = self.p5_out(w2[0, 1] * p5_x + w2[1, 1] * p5_td + w2[2, 1] * F.interpolate(p4_out, size=p5_x.shape[2:], mode="bilinear", align_corners=True))
        p6_out = self.p6_out(w2[0, 2] * p6_x + w2[1, 2] * p6_td + w2[2, 2] * F.interpolate(p5_out, size=p6_x.shape[2:], mode="bilinear", align_corners=True))
        p7_out = self.p7_out(w2[0, 3] * p7_x + w2[1, 3] * p7_td + w2[2, 3] * F.interpolate(p6_out, size=p7_x.shape[2:], mode="bilinear", align_corners=True))
...
if __name__ == '__main__':
    ...
    for i, c in enumerate([2, 3, 4]):
        f.append(torch.rand(1, c, 180//(2**i), 260//(2**i)).cuda(0))
    fpn = BiFPN([2, 3, 4])
    model = fpn.cuda(0)
    res = model(f)
    pass
